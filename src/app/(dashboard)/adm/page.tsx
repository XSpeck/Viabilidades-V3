...
// =====================
// Card: Importar CTOs Gmarx
// =====================
function ImportCtosGmarx() {
  const fileRef = useRef<HTMLInputElement>(null);
  const [ctosNoFirebase, setCtosNoFirebase] = useState<number | null>(null);
  const [loadingCount, setLoadingCount] = useState(true);
  const [file, setFile] = useState<File | null>(null);
  ...
  async function handleImport() {
    ...
    try {
      ...
      await importCtosGmarxToFirestore(ctos, (done, total) => {
        setProgress({ done, total });
      });

      setCtosNoFirebase(ctos.length);
      setResult({ ok: true, msg: `✅ ${ctos.length} CTOs Gmarx importadas com sucesso!` });
      setFile(null);
      setPreview(null);
      if (fileRef.current) fileRef.current.value = "";
    } catch (err) {
  ...
  return (
    <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
      <div className="flex items-center gap-3 px-5 py-4 border-b bg-gray-50">
        ...
        <div>
          <h3 className="font-semibold text-gray-800">CTOs Gmarx</h3>
          <p className="text-xs text-gray-500">KML das caixas disponíveis para clientes Gmarx</p>
        </div>
      </div>

      <div className="p-5 space-y-4">
        <div className="flex items-center justify-between bg-rose-50 border border-rose-100 rounded-lg px-4 py-3">
          <span className="text-sm text-gray-600">CTOs Gmarx no Firebase:</span>
          {loadingCount ? (
        ...
        <div
          onClick={() => fileRef.current?.click()}
          className="border-2 border-dashed border-gray-300 hover:border-rose-400 rounded-xl p-6 text-center cursor-pointer transition-colors group"
        >
          <Upload className="w-8 h-8 text-gray-400 group-hover:text-rose-500 mx-auto mb-2 transition-colors" />
        ...
        <button
          onClick={handleImport}
          disabled={!preview || importing}
          className="w-full bg-rose-600 hover:bg-rose-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-semibold py-2.5 rounded-lg text-sm transition-colors flex items-center justify-center gap-2"
        >
          {importing ? (
            <><Loader2 className="w-4 h-4 animate-spin" /> Importando...</>
          ) : (
            <><Upload className="w-4 h-4" /> Importar CTOs Gmarx</>
          )}
        </button>

        {(ctosNoFirebase ?? 0) > 0 && !importing && (
          <p className="text-xs text-gray-400 text-center">
            ⚠️ A importação substitui todas as CTOs Gmarx existentes.
          </p>
        )}
      </div>
    </div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {Array.from({ length: totalPaginas }, (_, i) => i + 1).map((p) => (
                      <button key={p} onClick={() => setPagina(p)}
                        className={`px-2.5 py-1 text-xs border rounded ${p === paginaAtual ? "bg-green-600 text-white border-green-600" : "hover:bg-gray-50"}`}>
                        {p}
                      </button>
                    ))}
